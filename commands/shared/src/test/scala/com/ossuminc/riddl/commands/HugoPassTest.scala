...
import com.ossuminc.riddl.utils.{Logger, SysLogger, ec, pc}
...
class HugoPassTest extends RunCommandOnExamplesTest() {
  ...
  def runHugo(outputDir: Path, tmpDir: Path): Assertion = {
    ...
    pc.log.info(s"Running hugo with cwd=$cwdFile, tmpDir=$tmpDir")
    ...
    proc.!<(logger) match {
      case 0 =>
        ...
        succeed
      case rc: Int if rc == 1 =>
        pc.log.info(s"hugo run failed with rc=$rc:\n  " ++ output.mkString("\n ", "\n  ", "\n"))
        succeed
      case rc: Int =>
        fail(
