...
| `${connector} crosses a context boundary but arrives at an inlet of ${owner}, which is inside ${ctx}; a context is the SINK for everything entering it` (Error) | `Declare an inlet on ${ctx} itself and connect to that; let its handlers dispatch or translate inward. Or arrive at an adaptor declared toward ${far} ('from' or, for the reply to its own ask, 'to') -- the adaptor IS ${ctx}'s boundary toward ${far}, in both directions.` |
| `${connector} crosses a context boundary but leaves from an outlet of ${owner}, which is inside ${ctx}; a context is the SOURCE for everything leaving it` (Error) | `Declare an outlet on ${ctx} itself and connect it from there; route the inner definition's outlet to it within ${ctx}. Or, if this crossing is ${ctx}'s integration with ${far}, leave from an adaptor declared toward ${far} ('to' or 'from') -- the adaptor IS the boundary for that pair, in both directions.` |
...
| `${entity} is declared event-sourced but has one state, no transitions, and none of its ${n} events is handled anywhere else -- the journal has no reader but the entity` (Advisory) | `If only the latest row matters, this is a CRUD entity: drop \`event-sourced\`. If history matters, model what reads it -- a projector or saga handling its events.` |
| `${entity} is not event-sourced but has ${s} states, ${m} transitions, and ${k} of its ${n} events are handled by other processors -- its history is being consumed from a store that keeps only the latest row` (Advisory) | `If consumers need the history, declare the entity \`event-sourced\`; if the latest row is all they need, this is fine as written.` |
| `every one of ${entity}'s ${n} events carries its whole state and every fold copies it back; replay reproduces the last row, so the journal buys durability of the log and nothing else` (Advisory) | `If that is the intent, fine -- the log is still durable. If the history is meant to carry meaning, record what CHANGED in each event rather than the whole state.` |
| `${entity} is event-sourced, but how ${k} of its ${n} handled events change its state is stated only in prose -- their replay has no defined semantics` (Completeness) | `State each fold: in every \`on event\` clause, \`set\` the state fields the event changes from the event's fields, or compute them, instead of \`prompt(…)\`/\`do\`.` |
| `'${kw}' targets ${field}, whose type '${type}' is not a collection; only a field with a '*', '+' or '{n,m}' cardinality can be appended to or removed from` | `Give ${field} a collection type (e.g. 'Item*'), or use 'set' to replace a single value.` |
| `'remove ... where ${key} == ...' names a field the elements of ${field} do not have[; its element type '${type}' is not a record]` | `Name one of the element record's fields as the key.` / `A keyed 'remove' needs a record element type with a field to match on; for a scalar collection use 'remove <value> from field F'.` |
| `${processor} is incomplete: it handles ${types} but declares no inlet to receive them on` (Missing; every processor kind — `entity-no-inlet` for an Entity, `stream-processor-no-inlet` otherwise) | `Declare an inlet on ${processor} typed with the message it handles, or with an alternation of them. A processor receives only through its OWN inlet -- a port on its context or on a sibling does not deliver to it.` |
| `${processor} is incomplete: it transmits ${types} but declares no outlet to transmit them on` (Missing; `entity-no-outlet` / `stream-processor-no-outlet`) | `Declare an outlet on ${processor} typed with the message it transmits, or with an alternation of them. Publishing goes out the processor's OWN outlet; its context's outlet is reached only by connecting the processor's outlet onward within the context.` |
...
| `${handler} in ${streamlet} handles messages but does not say what it does with them` (Completeness) | Say what the sink does with each message it handles: 'tell' it to an entity, 'log' it, 'put' it to an output, or give a code block. Prose alone leaves the behaviour unstated. |
...
| `${what} needs a collection but '${value}' is a '${type}'` (Error, `value-not-a-collection`, B5) | Name a field or value whose type is a collection -- 'T*', 'T+', 'set of T', 'sequence of T', a mapping or a table. |
| `'${table}' does not name a table of a schema in scope…` (Error, `repo-table-not-in-schema`, B2) | Name a data entry the schema declares with 'of <name> as record <R>', qualified by the schema when the repository has more than one. |
| `'${keyword}' into '${table}' needs a '${record}' but this is a '${actual}'` (Error, `stmt-store-value-not-table-record`, B2) | `Construct the record the schema stores: 'record ${record}(…)'.` |
| `'upsert' into '${table}' needs a key…` (Error, `stmt-upsert-needs-key`, B2) | `Add 'key on field ${record}.<idField>' to the schema, or use 'store' if every write is a new row.` |
| `'${field}' is not a field of '${record}', the record '${table}' stores` (Error, `stmt-update-field-not-in-row`, B2) | `Set one of: <the record's fields>.` |
| `'query' reads a repository's own storage and is only legal inside a repository` (Error, `value-query-outside-repository`, B2) | Ask the repository for what you need ('ask query … of repository …'), or move the read into the repository's handler. |
