...
class ASTTest extends AnyWordSpec with must.Matchers {
  ...
  "RootContainer" should {
    "be at location 0,0" in { RootContainer(Nil).loc mustBe Location(0, 0, "Root") }
    ...
    "contents" should {
      "contain all contents" in {
        ...
        val functions = Seq(Function(
          ...
          Option(Aggregation(
            Location(),
            Seq(Field(Location(), Identifier(Location(), "a"), Bool(Location())))
          )),
          Seq.empty[Example],
          None
        ))
        ...
        val invariants =
          Seq(Invariant(Location(), Identifier(Location(), "my_id"), True(Location()), None))
        val types = Seq(
