    ...
    "ValidationState" should {
      "parentOf" should {
        ...
        "not find the parent of a non-existant child" in {
          ...
          ValidationState(SymbolTable(domain)).parentOf(topic) mustBe
            RootContainer.empty
        }
        "checkNonEmpty" in {
          ValidationState(SymbolTable(RootContainer.empty))
            .checkNonEmpty(Nil, "foo", RootContainer.empty).msgs mustBe
        ...
        "checkOptions" in {
          ValidationState(SymbolTable(RootContainer.empty)).checkOptions(
            ...
            Location()
          ).msgs mustBe List(ValidationMessage(
            Location(),
            "Options should not be repeated",
            Validation.Error
          ))
          ValidationState(SymbolTable(RootContainer.empty))
            .checkOptions(List(1, 2, 3), Location()).msgs mustBe Nil
        }
      }
    }
  }
