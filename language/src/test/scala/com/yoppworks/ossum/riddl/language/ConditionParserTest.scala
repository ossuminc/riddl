  ...
  "ConditionParser" should {
    ...
    "accept and(true,false)" in {
      parseCondition("and(true,false)") { cond: Condition =>
        cond mustBe AndCondition(Location(1 -> 1),
    ...
    "accept xor(true,false)" in {
      parseCondition("xor(true,false)") { cond: Condition =>
        cond mustBe XorCondition(Location(1 -> 1),
          Seq(True(Location(1 -> 5)), False(Location(1 -> 10))))
      }
    }
    ...
    "accept comparison expressions" in {
      parseCondition("or(<(@a,42),<(@b,SomeFunc()))") { cond: Condition =>
        cond mustBe OrCondition(1 -> 1,
          Seq(
            Comparison(1 -> 4, lt,
              ValueCondition(1 -> 6, PathIdentifier(1 -> 7, Seq("a"))), LiteralInteger(1 -> 9,
                BigInt(42))),
            Comparison(1 -> 13, lt, ValueCondition(1 -> 15, PathIdentifier(1 -> 16, Seq("b"))),
            ...
            AndCondition(1 -> 4,
              Seq(
                NotCondition(1 -> 8, Comparison(1 -> 12, AST.eq,
                  ArbitraryCondition(LiteralString(1 -> 15, "sooth")), False(1 -> 24))),
                FunctionCallExpression(1 -> 32, PathIdentifier(1 -> 32, Seq("SomeFunc")),
