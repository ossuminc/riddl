...
/** The context for finding things within a given [Ccontainer]] or [[RiddlValue]] as found in the AST model. This
...
case class Finder(root: Container[RiddlValue]) {
  ...
  /** Search the `root` for a RiddlValue that matches the boolean expression
    * @param select
    *   The boolean expression to search for
    * @return
    *   A [[scala.Seq]] of the matching [[RiddlValue]]
    */
  ...
  /** THe return value for the [[findWithParents()]] function */
  type DefWithParents = Seq[(RiddlValue, Seq[RiddlValue])]

  /** Find a matching set of [[RiddlValue]] but return them with their parents
    *
    * @param select
    *   The boolean expression derived from a candidate [[RiddlValue]] that selects it to the result set
    * @return
    *   A [[Finder#DefWithParents]] that returns a [[scala.Seq]] of two-tuples with the [[RiddlValue]] a a [[scala.Seq]]
    *   of the parents of that value.
    */
  def findWithParents(
  ...
  /** Find definitions that are empty
    *
    * @return
    *   A [[scala.Seq]] of [[RiddlValue]], along with their parents that are empty
    */
