...
class BASTReader(
  ...
  private def readValue(): Value = {
    ...
    disc match
      ...
      case 18 => // B5: CollectionPredicate (revision 30)
        val loc = readLocation()
        val q = CollectionQuantifier.fromOrdinal(reader.readU8())
        val coll = readValue()
        val element = readIdentifierInline()
        CollectionPredicate(loc, q, coll, element, readValue())
      case 19 => // B5: CollectionFilter
        val loc = readLocation()
        val coll = readValue()
        val element = readIdentifierInline()
        CollectionFilter(loc, coll, element, readValue())
      case 20 => // B5: CountValue
        val loc = readLocation()
        CountValue(loc, readValue())
      case 21 => // B5: MembershipValue
        val loc = readLocation()
        val coll = readValue()
        MembershipValue(loc, coll, readValue())
