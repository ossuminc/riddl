...
trait ConditionParser extends ExpressionParser {
  ...
  def trueCondition[_: P]: P[True] = { P(location ~ IgnoreCase("true")).map(True)./ }

  def falseCondition[_: P]: P[False] = { P(location ~ IgnoreCase("false")).map(False)./ }
  ...
  def terminalExpressions[_: P]: P[Condition] = { P(trueCondition | falseCondition) }
