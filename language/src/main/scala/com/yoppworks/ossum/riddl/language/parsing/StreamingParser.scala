...
trait StreamingParser extends ReferenceParser with TypeParser with GherkinParser {
  ...
  def inlet[u: P]: P[Inlet] = {
    P(location ~ Keywords.inlet ~/ identifier ~ is ~ typeRef ~/
      (Readability.from ~ entityRef).? ~ briefly ~ description).map {
      tpl => (Inlet.apply _).tupled(tpl)
    }
  }

  def outlet[u: P]: P[Outlet] = {
    P(location ~ Keywords.outlet ~/ identifier ~ is ~ typeRef ~/
      (Readability.to ~ entityRef).? ~ briefly ~ description)
      .map { tpl => (Outlet.apply _).tupled(tpl) }
  }

  def toEntity[u: P]: P[EntityRef] = {
    P(Readability.to ~/ entityRef)
  }

  def fromEntity[u: P]: P[EntityRef] = {
    P(Readability.from ~/ entityRef)
  }

  def source[u: P]: P[Processor] = {
    P(
      location ~ Keywords.source ~/ identifier ~ is ~ open ~
        (outlet.map(Seq(_)) | undefined(Seq.empty[Outlet])) ~
        close ~ testedWithExamples ~ briefly ~ description
    ).map {
      case (location, id, outlets, examples, brief, description) =>
        Processor(location, id, Source(location), Seq.empty[Inlet], outlets,
  ...
  def sink[u: P]: P[Processor] = {
    P(
      location ~ Keywords.sink ~/ identifier ~ is ~ open ~
        (inlet.map(Seq(_)) | undefined(Seq.empty[Inlet])) ~
        close ~ testedWithExamples ~ briefly ~ description
    ).map {
      case (location, id, inlets, examples, brief, description) =>
        Processor(location, id, Sink(location), inlets, Seq.empty[Outlet],
          examples, brief, description)
    }
  }
  ...
  def flow[u: P]: P[Processor] = {
    P(
      location ~ Keywords.flow ~/ identifier ~ is ~ open ~
        ((inlet.map(Seq(_)) ~ outlet.map(Seq(_))) | undefined((Seq.empty[Inlet], Seq
          .empty[Outlet]))) ~
        close ~ testedWithExamples ~ briefly ~ description
    ).map {
      case (location, id, (inlet, outlet), examples, brief, description) =>
        Processor(location, id, Flow(location), inlet, outlet,
          examples, brief, description)
    }
  }
  ...
  def split[u: P]: P[Processor] = {
    P(
      ...
      case (location, id, (inlets, outlets), examples, brief, description) =>
        Processor(location, id, Split(location), inlets, outlets,
          examples, brief, description)
    }
  }
  ...
  def merge[u: P]: P[Processor] = {
    P(
      ...
      case (location, id, (inlets, outlets), examples, brief, description) =>
        Processor(location, id, Merge(location), inlets, outlets,
          examples, brief, description)
    }
  }
    ...
    P(
      location ~ Keywords.multi ~/ identifier ~ is ~ open ~
        ((inlet.rep(2) ~ outlet.rep(2)) |
          undefined((Seq.empty[Inlet], Seq.empty[Outlet]))) ~
        close ~ testedWithExamples ~ briefly ~ description
    ).map {
      case (location, id, (inlets, outlets), examples, brief, description) =>
