...
trait FeatureParser extends CommonParser {
  ...
  def givens[_: P]: P[Seq[Given]] = {
    P(
      (location ~ IgnoreCase("given") ~/ docBlock)
        .map(tpl => (Given.apply _).tupled(tpl)) ~
        (location ~ IgnoreCase("and") ~/ docBlock)
          .map(tpl => (Given.apply _).tupled(tpl)).rep(0)
  ...
  def whens[_: P]: P[Seq[When]] = {
    P(
      (location ~ IgnoreCase(Keywords.when) ~/ docBlock)
        .map(tpl => (When.apply _).tupled(tpl)) ~
        (location ~ IgnoreCase("and") ~/ docBlock)
          .map(tpl => (When.apply _).tupled(tpl)).rep(0)
    ).map { case (initial, remainder) => initial +: remainder }
  }
  ...
  def thens[_: P]: P[Seq[Then]] = {
    P(
      (location ~ IgnoreCase(Keywords.then_) ~/ docBlock)
        .map(tpl => (Then.apply _).tupled(tpl)) ~
        (location ~ IgnoreCase(Readability.and) ~/ docBlock)
          .map(tpl => (Then.apply _).tupled(tpl)).rep(0)
    ).map { case (initial, remainder) => initial +: remainder }
  }

  def elses[_: P]: P[Seq[Else]] = {
    P(
      (location ~ IgnoreCase(Keywords.else_) ~/ docBlock)
        .map(tpl => (Else.apply _).tupled(tpl)) ~
        (location ~ IgnoreCase(Readability.and) ~/ docBlock)
          .map(tpl => (Else.apply _).tupled(tpl)).rep(0)
    ).?.map {
  ...
  def namedExample[_: P]: P[Example] = {
    P(
      location ~ IgnoreCase(Keywords.example) ~/ identifier ~ open ~/ givens ~
        whens ~ thens ~ elses ~ close ~ description
    ).map { tpl => (Example.apply _).tupled(tpl) }
  }

  def examples[_: P]: P[Seq[Example]] = {
    implicitExample.map(Seq(_)) | namedExample.rep(1)
  }
    ...
    P(
      location ~ IgnoreCase(Keywords.feature) ~/ identifier ~ is ~ open ~
        (undefined.map(_ => (None, Seq.empty[Example])) |
          (background.? ~ examples)) ~ close ~ description
    ).map { case (loc, id, (bkgrnd, examples), desc) =>
