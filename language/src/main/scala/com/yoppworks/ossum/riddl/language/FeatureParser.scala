...
trait FeatureParser extends CommonParser {
  ...
  def exampleBody[_: P]: P[(Seq[Given], Seq[When], Seq[Then], Seq[Else])] = {
    P(
  ...
  def examples[_: P]: P[Seq[Example]] = { implicitExample.map(Seq(_)) | namedExample.rep(1) }
  ...
  def feature[_: P]: P[Feature] = {
    P(
      location ~ IgnoreCase(Keywords.feature) ~/ identifier ~ is ~ open ~
        (undefined.map(_ => (None, Seq.empty[Example])) | (background.? ~ examples)) ~ close ~
        description
