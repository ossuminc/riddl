...
object DefinitionValidator {
  ...
  def validate(
    ...
    definition match {
      ...
      case i: Include[Definition] @unchecked => i.contents.foldLeft(state) {
  ...
  private def validateField(
    ...
    state.checkDefinition(parents, f)
  ...
  private def validateExample(
    ...
    state.checkDefinition(parents, e).checkExample(e, parents)
      .checkDescription(e)
  }
  ...
  private def validateInvariant(
    ...
    state.checkDefinition(parents, i)
  ...
  private def validateAuthorInfo(
    ...
    state.checkDefinition(parents, ai)
  ...
  private def validateType(
    ...
    state.addType(t)
    state.checkDefinition(parents, t).check(
  ...
  private def validateState(
    ...
    state.checkContainer(parents, s)
  ...
  private def validateFunction(
    ...
    state.addFunction(f).checkContainer(parents, f)
  ...
  private def validateOnClause(
    ...
    (oc match {
      ...
      case omc @ OnMessageClause(_, msg, from, _, _, _) => state
  ...
  private def validateInclude[T <: Definition](
    ...
    state.check(i.nonEmpty, "Include has no included content", Error, i.loc)
  ...
  private def validateEntity(
    ...
    state.addEntity(e).checkContainer(parents, e)
  ...
  private def validateProjection(
    ...
    state.checkContainer(parents, p).check(
      p.types.exists { typ =>
        typ.typ match {
  ...
  private def validateSaga(
    ...
    state.checkContainer(parents, s).check(
      s.nonEmpty && s.sagaSteps.size >= 2,
      "Sagas must define at least 2 steps",
      Error,
      s.loc
    ).check(
      s.nonEmpty && s.sagaSteps.size >= 2 && s.sagaSteps.map(_.id.value)
        .allUnique,
  ...
  private def validateContext(
    ...
    state.checkContainer(parents, c)
  ...
  private def validateInput(
    ...
    state.checkDefinition(parents, in)
      .checkMessageRef(in.putIn, in, parents, CommandCase).checkDescription(in)
  }
  ...
  private def validateOutput(
    ...
    state.checkDefinition(parents, out)
  ...
  private def validateActor(
    ...
    state.checkDefinition(parents, actor).checkThat(actor.is_a.isEmpty) { vs =>
      vs.addMissing(
        actor.loc,
  ...
  private def validateStoryCase(
    ...
    state.checkDefinition(parents, sc).stepIf(sc.interactions.nonEmpty) { st =>
      ...
      vs.checkThat(sc.interactions.isEmpty)(
        _.addMissing(sc.loc, s"${sc.identify} doesn't define any interactions")
      )
    }.checkDescription(sc)
  }
}
