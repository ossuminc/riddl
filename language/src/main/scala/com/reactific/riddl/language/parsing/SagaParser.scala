...
/** SagaParser Implements the parsing of saga definitions in context definitions.
  */
private[parsing] trait SagaParser extends ReferenceParser with ActionParser with FunctionParser with StreamingParser {

  private def sagaStep[u: P]: P[SagaStep] = {
    P(
      location ~ Keywords.step ~/ identifier ~ is ~ open ~ actions ~ close ~
        Keywords.reverted ~ Readability.by.? ~ open ~ actions ~ close ~
        briefly ~ description
