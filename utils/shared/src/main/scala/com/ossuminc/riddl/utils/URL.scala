...
case class URL(scheme: String = "", authority: String = "", basis: String = "", path: String = "") extends AnyRef {
  ...
  @JSExport
  def toExternalForm: String = toString

  /** Get the path component of the URL */
  @JSExport
  def toFullPathString: String = "/" + basis + "/" + path

  @JSExport
  def root: URL = URL(toBasisString)
  ...
  @JSExport
  def resolve(pathElement: String): URL = {
    require(!pathElement.startsWith("/"), "Invalid path element starts with: /")
    val p =
      if path.isEmpty then pathElement
...
object URL {
  ...
  private val httpPattern = s"""https?://$authorityPattern/$pathPattern?([?#][^#?/]+)?""".r
  private val filePattern = s"file://$authorityPattern/$pathPattern".r
  ...
  def fromFullPath(path: String): URL = {
    ...
    if path.endsWith("/") then URL(fileScheme, "", path.drop(1).dropRight(1))
    else
