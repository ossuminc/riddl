...
import com.yoppworks.ossum.riddl.language.BuildInfo
import com.yoppworks.ossum.riddl.language.FormattingOptions
...
object RiddlOptions {
  ...
  def parse(args: Array[String]): Option[RiddlOptions] = {
    OParser.runParser(RiddlOptions.parser, args, RiddlOptions(), setup) match {
      case (result, effects) =>
        ...
        result match {
          case Some(options) => Some(options.copy(
  ...
  def configPath(f: OptionPlacer[File]): OParser[File, RiddlOptions] = {
    opt[File]('c', "configuration-file").optional().action((v, c) => f(v, c))
  ...
  private val parser: OParser[Unit, RiddlOptions] = {
    OParser.sequence(
      ...
      cmd("parse").action((_, c) => c.copy(command = Parse)).children(inputFile((v, c) =>
        c.copy(parseOptions = c.parseOptions.copy(inputPath = Some(v.toPath)))
      ...
      cmd("validate").action((_, c) => c.copy(command = Validate)).children(inputFile((v, c) =>
        c.copy(validateOptions = c.validateOptions.copy(inputPath = Some(v.toPath)))
      ...
      cmd("reformat").action((_, c) => c.copy(command = Prettify)).children(
        inputFile((v, c) =>
          c.copy(reformatOptions = c.reformatOptions.copy(inputPath = Some(v.toPath)))
        ),
        outputDir((v, c) =>
          c.copy(reformatOptions = c.reformatOptions.copy(outputPath = Some(v.toPath)))
        ),
      ...
      cmd("hugo").action((_, c) => c.copy(command = Hugo)).children(
        inputFile((v, c) => c.copy(hugoOptions = c.hugoOptions.copy(inputPath = Some(v.toPath)))),
        outputDir((v, c) => c.copy(hugoOptions = c.hugoOptions.copy(outputPath = Some(v.toPath)))),
        configPath((v, c) => c.copy(hugoOptions = c.hugoOptions.copy(configPath = Some(v.toPath)))),
        baseUrl((v, c) => c.copy(hugoOptions = c.hugoOptions.copy(baseUrl = Some(v)))),
        projectName((v, c) => c.copy(hugoOptions = c.hugoOptions.copy(projectName = Some(v))))
