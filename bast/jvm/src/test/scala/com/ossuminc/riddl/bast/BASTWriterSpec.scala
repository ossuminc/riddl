...
class BASTWriterSpec extends AbstractRunPassTest {
  ...
  "BASTWriter" should {
      ...
      inputFuture.map { input =>
        ...
        parseResult match {
          ...
          case Right(root: Root) =>
            ...
            val bastWriter = BASTWriter.creator()
            ...
            // Get the BAST output
            val bastOutput = result.outputOf[BASTOutput](BASTWriter.name)
      ...
      inputFuture.map { input =>
        ...
        parseResult match {
          case Right(root: Root) =>
            ...
            val bytes = result.outputOf[BASTOutput](BASTWriter.name).get.bytes
    ...
    "serialize small RIDDL file from riddl-examples" in { (td: TestData) =>
      val examplesPath = Paths.get("/Users/reid/Code/ossuminc/riddl-examples")
      if !Files.exists(examplesPath) then
        info("riddl-examples not found, skipping test")
        succeed
      else
        ...
        inputFuture.map { input =>
          ...
          parseResult match {
            case Right(root: Root) =>
              ...
              info(s"ToDoodles: ${output.nodeCount} nodes, ${output.bytes.length} bytes")
              succeed
    ...
    "serialize large project from riddl-examples" in { (td: TestData) =>
      val examplesPath = Paths.get("/Users/reid/Code/ossuminc/riddl-examples")
      if !Files.exists(examplesPath) then
        info("riddl-examples not found, skipping test")
        succeed
      else
        // ReactiveBBQ is a larger project with multiple files
        ...
        inputFuture.map { input =>
          val start = System.nanoTime()
          val parseResult = TopLevelParser.parseInput(input, true)
          val parseTime = (System.nanoTime() - start) / 1_000_000.0
          ...
          parseResult match {
            case Right(root: Root) =>
              ...
              info(s"ReactiveBBQ domain:")
              info(f"  Parse time: $parseTime%.2f ms")
              info(f"  BAST serialization time: $bastTime%.2f ms")
              info(s"  Nodes: ${output.nodeCount}")
              info(s"  Size: ${output.bytes.length / 1024} KB")
              info(s"  String table: ${output.stringTableSize} strings")
              info(f"  Ratio: ${parseTime / bastTime}%.1fx faster write than parse")
