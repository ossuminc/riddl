...
abstract class HugoTranslateExamplesBase extends ValidatingTest {
  ...
  def genHugo(projectName: String, source: String): Seq[Path] = {
    ...
    Riddl.parseAndValidate(sourcePath,commonOptions) match {
      case Right(root) =>
        ht.doTranslation(root, SysLogger(), commonOptions, htc)
      case Left(messages) =>
        require(messages.isEmpty, s"Non empty messages: ${messages}")
        Seq.empty[Path]
    }
  }
