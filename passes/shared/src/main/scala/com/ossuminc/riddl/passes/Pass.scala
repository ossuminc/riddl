...
abstract class Pass(@unused val in: PassInput, val out: PassesOutput) {
  ...
  /** Pre-process the root before actually traversing. This is also a signal that the processing is about to start.
    * @param root
    *   A [[com.ossuminc.riddl.language.AST.Root]] node which encapsulates the whole model. This is the state of 
    *   the Root as of the last pass.
    * @return
...
abstract class CollectingPass[ET](input: PassInput, outputs: PassesOutput) extends Pass(input, outputs) {
  ...
  /** The processing method called at each node, similar to [[Pass.process]] but modified to
    * return an sequence of the collectable, [[ET]].
    *
    * @param definition
