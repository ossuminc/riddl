...
trait PassOutput {
  ...
  /** The new [[Root]] of the model, if the [[Pass]] modified it in [[Pass.preProcess()]] */
  def root: Root

  /** Any [[Messages]] that were generated by the pass */
...
abstract class Pass(@unused val in: PassInput, val out: PassesOutput) {
  ...
  /** Pre-process the root before actually traversing. This is also a signal that the processing is about to start.
    * @param root
    *   A [[Root]] node which encapsulates the whole model. This is the state of the Root as of the last pass
    * @return
  ...
  /** Generate the output of this Pass. This will only be called after all the calls to process have completed.
    * @param root
    *   The new [[Root]] of the model that the pass computed
    * @return
...
abstract class CollectingPass[F](input: PassInput, outputs: PassesOutput) extends Pass(input, outputs) {
  ...
  override def result(root: Root): CollectingPassOutput[F]
}
