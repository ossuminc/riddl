...
/** [1.2]: an event emitted ONLY through a widened operand — a `let`-local rather than a bare
  ...
  *
  * `emittedMessageTypes` is a single flat `Finder` sweep over the whole root, so a `send`/`tell` it
  * finds carries no notion of which clause it came from, let alone that clause's `let` scope. It
  * therefore resolved operands with the NARROW `operandType` and could not see through a `ValueRef`
  * at all. The consequence was a FALSE POSITIVE: a correlation folding such an event drew "no
  * processor in this model emits it" while something plainly did.
  *
    ...
    "not be reported as emitted by nothing" in { (td: TestData) =>
      parseAndValidateInput(RiddlParserInput(src, td), shouldFailOnErrors = false) {
        case (_, _, msgs: Messages.Messages) =>
          val unemitted =
            msgs.filter(m => m.message.contains("Shipped") && m.message.contains("emit"))
