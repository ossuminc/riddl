            ...
            case Right(originalRoot: Root) =>
              ...
              def findIncludes(value: RiddlValue, depth: Int = 0): Unit = {
                ...
                value match {
                  case i: Include[?] =>
                    println(
                      s"${indent}Include at ${i.loc}: origin=${i.origin}, contents count=${i.contents.length}"
                    )
              ...
              for pos <- HEADER_SIZE until stringTableOffset do
                ...
                if b == NODE_INCLUDE || b == (NODE_INCLUDE | 0x80) then
                  ...
                  println(
                    f"  Position $pos%4d: tagByte=$b%3d (with metadata flag: ${(b & 0x80) != 0})"
                  )
              ...
              for pos <- HEADER_SIZE until stringTableOffset do
                ...
                if b == FILE_CHANGE_MARKER && pos > HEADER_SIZE then
                  ...
                  println(
                    f"  Position $pos%4d: FILE_CHANGE_MARKER, stringIdx=$stringIdx, next tag=$tagAfter"
                  )
                end if
              end for
