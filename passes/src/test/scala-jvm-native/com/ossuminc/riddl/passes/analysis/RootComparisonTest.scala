...
/** Tests for [[RootComparison]] — the deterministic, model-free `Root` similarity API used by
  * ossum-gen's compositional-fidelity benchmark.
  */
class RootComparisonTest extends AbstractValidatingTest {
  ...
  "RootComparison" should {
    ...
    "report per-kind counts that match a manual StatsPass reading" in { (td: TestData) =>
      val root = parse(base, "base")
    ...
    "count breadth as top-level containers, not the Root wrapper" in { (td: TestData) =>
    ...
    "keep a renamed-but-structurally-equal model high on structure" in { (td: TestData) =>
      val a = parse(base, "base")
    ...
    "render a Markdown report with counts, names, metrics, and score" in { (td: TestData) =>
      val a = parse(base, "base")
      val b = parse(renamed, "renamed")
    ...
    "work on real, non-trivial in-repo models (self = 1.0 > cross)" in { (td: TestData) =>
