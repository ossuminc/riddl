...
/** Tests for the CommonOptions.provideTips mechanism: every validation/ resolution message can
  * carry a remediation `suggestion`, which is retained (and rendered by Message.format) only when
  * provideTips is enabled.
  */
class ProvideTipsTest extends AbstractValidatingTest {

  /** A model that reliably emits at least one warning carrying a suggestion (the "should have a
    * description" MissingWarning on each definition).
    */
  ...
  /** A model whose entity defines no command types, exercising one of the advisory completeness
    * checks that only fire when provideTips is on.
    */
  ...
  "provideTips" should {
    ...
    "strip remediation suggestions from messages by default" in { (td: TestData) =>
      withProvideTips(on = false) {
        parseAndValidate(describedModel, "test", shouldFailOnErrors = false) { (_, _, msgs) =>
    ...
    "emit advisory entity-completeness checks only when enabled" in { (td: TestData) =>
      ...
      withProvideTips(on = false) {
        parseAndValidate(entityModel, "test", shouldFailOnErrors = false) { (_, _, msgs) =>
          hasNoCommandTypesWarning(msgs) mustBe false
        }
      }
