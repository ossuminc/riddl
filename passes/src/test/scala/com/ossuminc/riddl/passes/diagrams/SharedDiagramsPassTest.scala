...
abstract class SharedDiagramsPassTest(using PlatformContext) extends AbstractValidatingTest {
  ...
  "DiagramsPass" must {
    ...
    /* [2.4] Streamlet -> Processor, then [4.1]. A data flow diagram was built from a port-to-owner
     * walk that matched the concrete `Streamlet` case class, falling back to the PORT itself.
     * Since the unified processor model any Processor may declare its own ports, so a flow between
     * two entities' ports drew arrows from an outlet to an inlet, with neither entity appearing in
     * the diagram at all. */
    "draw a data flow between processors that own the ports, not the ports themselves" in {
      (td: TestData) =>
        ...
        parseAndValidateAggregate(input) { passesResult =>
          ...
          // [4.1], RULED 2026-08-17: `streamlets` means every PORT-BEARING processor, so the two
          // entities ARE its answer. This assertion said `data.streamlets mustBe empty` under the
          // previous reading, with a separate `portBearing` field holding them; that field is gone.
          data.streamlets.map(_.id.value).toSet mustBe Set("Source", "Target")
        }
    }
