...
/** Output from the Symbols Pass
  ...
  * @param symTab
  *   The SymbolTable that handles identifier translation to definitions. It holds ONLY the user's
  *   model, so anything that enumerates it (`AnalysisResult.domains`, `UseCaseWitnessPass`, …) sees
  *   the model and nothing else.
  * @param predefinedSymTab
  *   The symbols of the predefined `Riddl` standard module, kept in a SEPARATE table so they never
  *   pollute an enumeration of the user's model. Every lookup consults `symTab` first and falls
  *   back to this only when the name is absent there — which is exactly the rule that a user
  *   definition of the same name WINS.
  * @param predefinedParentage
  *   Parentage for the predefined definitions, separate for the same reason.
  */
case class SymbolsOutput(
  ...
  predefinedSymTab: SymTab = Symbols.emptySymTab,
  predefinedParentage: Parentage = Symbols.emptyParentage
) extends PassOutput {

  /** The symbol-table items for a leaf name: the user's model first, the predefined standard module
    * only as a fallback. A name the user defined shadows the predefined one completely.
    */
  private def itemsFor(leafName: String): Option[SymTabItems] =
    symTab.get(leafName).orElse(predefinedSymTab.get(leafName))
  ...
  /** Get the parent of a definition
    ...
    */
  def parentOf(definition: Definition): Option[Branch[?]] =
    parentsOf(definition).headOption
  ...
  def parentsOf(definition: Definition): Parents = {
    parentage.get(definition).orElse(predefinedParentage.get(definition)) match {
      case Some(list) => list
  ...
  /** Find the bounded referent in which a give named value is defined
    * @param definition
    *   The [[com.ossuminc.riddl.language.AST.Definition]] whose
    *   [[com.ossuminc.riddl.language.AST.Context]] should be determined
    * @return
  ...
  /** The result of a lookupSymbol request A lookupSymbol request returns a list of tuples that
    * contain the generic definition, as a Definition, and, if the definition matches the type of
    * interest, D, then an Option[D] for convenience.
    */
  ...
  /** Look up a symbol in the table
    *
    * @param id
    *   The multi-part identifier of the symbol, from leaf to root, that is from most nested to
    *   least nested.
    * @tparam D
    *   The expected type of definition
    * @return
    *   A list of matching definitions of 2-tuples giving the definition as a Definition type and
    *   optionally as the requested type
    */
